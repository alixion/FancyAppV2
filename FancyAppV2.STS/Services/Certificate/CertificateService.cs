...
namespace FancyAppV2.STS.Services.Certificate
{
    public static class CertificateService
    {
        public static async Task<(X509Certificate2 ActiveCertificate, X509Certificate2 SecondaryCertificate)>
            GetCertificates(CertificateConfiguration certificateConfiguration)
        {
            ...
            if (certificateConfiguration.UseLocalCertStore)
            {
                ...
                var storeCerts = store.Certificates.Find(X509FindType.FindByThumbprint,
                    certificateConfiguration.CertificateThumbprint, false);
