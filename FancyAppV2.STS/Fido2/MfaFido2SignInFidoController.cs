    ...
    public class MfaFido2SignInFidoController : Controller
    {
            ...
            try
            {
                ...
                if (identityUser == null)
                {
                    throw new InvalidOperationException(
                        _sharedLocalizer["FIDO2_UNABLE_TO_LOAD_2FA_AUTHENTICATED_USER"]);
                }
        ...
        public async Task<JsonResult> MakeAssertion([FromBody] AuthenticatorAssertionRawResponse clientResponse)
        {
            try
            {
                ...
                var res = await _lib.MakeAssertionAsync(clientResponse, options, creds.PublicKey, storedCounter,
                    callback);
